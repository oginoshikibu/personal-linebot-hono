...
export const handleDinnerPostback = async (
  ...
  try {
    ...
    if (action === "dinner_cancel") {
      await sendTextMessage(user.lineId, "夕食の予定入力をキャンセルしました。昼食の予定のみ保存されました。");
      ...
      if (date.getTime() === today.getTime() || date.getTime() === tomorrow.getTime()) {
        ...
        logger.info(`他のユーザーに通知: ${dateStr}, 昼食のみ`, { userId: user.lineId });
      }
    ...
    logger.info(`夕食の予定を保存: ${dateStr}, ${attendance}`, { userId: user.lineId });
    
    // 夕食の予定を保存した旨のメッセージを送信
    await sendTextMessage(user.lineId, `${dateText}の夕食: ${getAttendanceText(attendance)}`);
    
    // 登録完了メッセージを送信
    await sendTextMessage(user.lineId, `${dateText}の食事予定の登録が完了しました。`);
    ...
    if (date.getTime() === today.getTime() || date.getTime() === tomorrow.getTime()) {
      // TODO: 他のユーザーに通知する処理を追加
      logger.info(`他のユーザーに通知: ${dateStr}, 昼食と夕食`, { userId: user.lineId });
    }
